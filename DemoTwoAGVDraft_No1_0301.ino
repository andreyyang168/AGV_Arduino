...
String DestinationPos = DownTerminal; //set initial DestinationPos. Confirm Before Test!!
...
float DestinationAng = DownDockAng; //set  initial DestinationAng. Confirm Before Test!!
...
-----------------------------------------------------------------------*/
  ...
  timer_Barcode.update();     // readBarcode 200ms
  timer_Velocity.update();    // calVelocity 300ms
  timer_HMC5883L.update();    // headingAngle 450ms
  timer_SR04.update();        // ultrasonicDetect 500ms
  timer_IRdist.update();      // IR_distance 1000ms
  timer_Battery.update();     // ultrasonicDetect 10000ms
...
//        motor3.setSpeed(Output_3);
        motor3.setSpeed(85 + PWMcomp3);
//        motor3.setSpeed(140 + PWMcomp3); //original
//----------------------------------------------------
...
//        motor1.setSpeed(Output_1);
        motor1.setSpeed(74 + PWMcomp1);
//        motor1.setSpeed(110 + PWMcomp1); //original
//----------------------------------------------------
        motor4.run(RELEASE);
}
void slowGoForward(){
...
//        PID_3.SetOutputLimits(0, 195);
//        motor3.setSpeed(Output_3);
//        motor3.setSpeed(57 + PWMcomp3);
        motor3.setSpeed(77+PWMcomp3);
//----------------------------------------------------
        motor1.run(BACKWARD);
...
//        PID_1.SetOutputLimits(0, 155);
//        motor1.setSpeed(Output_1);
//        motor1.setSpeed(40 + PWMcomp1);
        motor1.setSpeed(62+PWMcomp1);
//----------------------------------------------------
        motor4.run(RELEASE);
}
void GoBackward() {
...
//        motor1.setSpeed(60);//without PID
        motor1.setSpeed(83.25);//without PID
//---------------------------------------------------- 
        motor3.run(BACKWARD);
//        Setpoint_3 = 5.5;
//        PID_3.SetTunings(Kp1, Ki1, Kd1);
...
//        motor3.setSpeed(77.8);//without PID
        motor3.setSpeed(106.68);//without PID
//----------------------------------------------------
        motor4.run(FORWARD);
//        Setpoint_4 = 9;
...
//        motor4.setSpeed(130);// without PID
        motor4.setSpeed(161);// without PID
}
void GoCrab_Left() {
        motor1.run(FORWARD);
        Setpoint_1 = 5.5; // cm/sec
...
//  Serial.println("Ultra");
  ...
  if(cmMsec_F<=4.5) cmMsec_F=9.1;
//  Serial.print("MS: ");  Serial.print(microsec_F);
//  Serial.print(", F_CM: "); Serial.print(cmMsec_F);
...
//      XB_BCD.substring(0,6); //取得6 char的String, The starting index is inclusive, but the optional ending index is exclusive
...
//  else if( barcodePrev.substring(4)>barcodeNow.substring(4) && (負Y角度-5)<=xDegrees<=(負Y角度+5)) //行經路徑為-Y ex:09>08
  ...
  if( (UpTerminal_Y.toInt()-barcodeNow_Y.toInt()) > (barcodeNow_Y.toInt()-DownTerminal_Y.toInt()) && LowDockOcpd==0 && UpDockOcpd==0){
      DestinationPos = DownTerminal;
      DestinationAng = DownDockAng;
      Serial.print('L'); //inform the other 
  }
  /*離upTerminal比較近*/
  else{
      DestinationPos = UpTerminal;
      DestinationAng = UpDockAng;
      Serial.print('P'); //inform the other 
  }
...
void RFIDevent(){
  ...
  */
  ...
  else if(memcmp(AGV2_Tag, mfrc522.uid.uidByte, mfrc522.uid.size) == 0)
  {
      if(mfrc522.uid.size != 0){      // double check
  //      Serial.println("detect AGV2_Tag (tag4)~~~~~begin to makeWay()~~~~~~~~~~~");
...
//  Serial.print(" makeWay");
  ...
  encoderCount_BeforeTurn4 = encoderCount_4; //record current position(angle)
  encoderCount_AfterTurn4 = encoderCount_BeforeTurn4 + 2300;   //offset 2000 pulse 
  while(encoderCount_4 < encoderCount_AfterTurn4 ){
      GoCrab_Right();
...
void turn180(){
  ...
  while( encoderCount_4 < encoderCount_BeforeTurn4+3050){ //強制轉180度  
    Rotate_CCW();
//    Serial.println(encoderCount_4);
  }
  
  if( encoderCount_4 >= encoderCount_BeforeTurn4+3050){ //=>"2920" with 11.1V Batt/ "3100" with 7.4V Batt
      Rotate_CW(); //反向剎車
      delay(100);
      Stop();
//      Serial.println("180~~~~~~~~~~~~~~~~~~~~~~~~~~~");
